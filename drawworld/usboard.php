...
<!--banner-info-->
	<div class="banner-info">
		<h1><a href="usboard.php">게시판</a></h1>
	</div>
</div>
			<!-- //get-->
			<section>
  ...
  <div class="container">
		...
		<div class="row" style="padding-top:60px">
			<div style="float: right;">
				<form name="selectTable" method="get" action='usboard.php'>
					<table>
						<tr>
							<td>정렬 방식</td>
    ...
    <div class="row">
        ...
        $page = 1;
				...
				if(isset($_GET["searching_type"])){
					if($searching_type == "name"){
						...
						$sql = "select * from usboard where $searching_type = '$searching'";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}else{
						$searching = $searching.'%';
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from usboard where $searching_type like '$searching'";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}
				}else{
					if($setting == "num"){
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from usboard order by num desc";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}else if($setting == "hit"){
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						...
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from usboard order by recommand desc";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}
				}
      ...
      ?>
        <tr>
          ...
          <td><a href="board_view.php?num=<?=$num?>&page=<?=$page?>&board_type=usboard"><?=$subject?><?php if($file_image!=" "){echo"&nbsp&nbsp&nbsp<i class='fas fa-file'></i>";} ?></a></td>
    ...
    {
    $new_page = $page-1;
    echo "<li><a class='frist' href='usboard.php?page=$new_page&setting=$setting'>Prev</a><a href='#'></a></li>";
    }
    else
    echo "&nbsp;";
    ...
    if ($page == $i)     // 현재 페이지 번호 링크 안함
    {
      ...
      {
        echo "<li><a href='usboard.php?page=$i&setting=$setting'> $i </a></li>";
      }
    }
    if ($total_page>=2 && $page != $total_page)
    {
    $new_page = $page+1;
      echo "<li><a class='last' href='usboard.php?page=$new_page&setting=$setting'>Next</a></li>";
    }
    ?>
	</ul>
  </div>
	<div class="col-md-4">
		<form action="usboard.php" method="get">
			<table>
				<tr>
					<td>
