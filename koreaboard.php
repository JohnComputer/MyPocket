...
<!--banner-info-->
	<div class="banner-info">
		<h1><a href="koreaboard.php">게시판</a></h1>
	</div>
</div>
			<!-- //get-->
			<section>
  ...
  <div class="container">
		...
		<div class="row" style="padding-top:60px">
			<div style="float: right;">
				<form name="selectTable" method="get" action='koreaboard.php'>
					<table>
						<tr>
							<td>정렬 방식</td>
    ...
    <div class="row">
      <table id="customers" style="padding-top:20px">
        <tr>
          ...
          <th style="width:200px" class="hidden-xs">등록일</th>
        ...
        $page = 1;
				...
				if(isset($_GET["searching_type"])){
					if($searching_type == "name"){
						...
						$sql = "select * from koreaboard where $searching_type = '$searching'";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}else{
						$searching = $searching.'%';
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from koreaboard where $searching_type like '$searching'";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}
				}else{
					if($setting == "num"){
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from koreaboard order by num desc";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}else if($setting == "hit"){
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						...
						$con = mysqli_connect("localhost", "user", "12345", "drawworld");
						$sql = "select * from koreaboard order by recommand desc";
						$result = mysqli_query($con, $sql);
						$total_record = mysqli_num_rows($result); // 전체 글 수
					}
				}
      ...
      ?>
        <tr>
          ...
          <td><a href="board_view.php?num=<?=$num?>&page=<?=$page?>&board_type=koreaboard"><?=$subject?><?php if($file_image!=" "){echo"&nbsp&nbsp&nbsp<i class='fas fa-file'></i>";} ?></a></td>
          <td style="text-align: center;"><?php echo "<img style='width:20px; height:15px;' src='./data/".$imgsrc."' width=100% height=100%/>";?><?=$name?></td>
          <td class="hidden-xs" style="text-align: center;"><?=$regist_day?></td>
    ...
    {
    $new_page = $page-1;
    echo "<li><a class='frist' href='koreaboard.php?page=$new_page&setting=$setting'>Prev</a><a href='#'></a></li>";
    }
    else
    echo "&nbsp;";
    ...
    if ($page == $i)     // 현재 페이지 번호 링크 안함
    {
      ...
      {
        echo "<li><a href='koreaboard.php?page=$i&setting=$setting'> $i </a></li>";
      }
    }
    if ($total_page>=2 && $page != $total_page)
    {
    $new_page = $page+1;
      echo "<li><a class='last' href='koreaboard.php?page=$new_page&setting=$setting'>Next</a></li>";
    }
    ?>
	</ul>
  </div>
	<div class="col-md-4">
		<form action="koreaboard.php" method="get">
			<table>
				<tr>
					<td>
